...
*  Salinity: ANOVA not significant.  Possible weak summer trend.  
   *  "No trend"   
*  DO:  No significant terms by ANOVA, so don't want to interpret marginal 
   means.  
   *  "No trend"   
...
```{r tall_annot, fig.height = 9.5, fig.width = 3.5}
p_jit +
...
```{r tall_annot_tick, fig.height = 9.5, fig.width = 3.5}
p_jit +
  scale_color_manual(values = cbep_colors2()[c(1,5,2)],
                     name = '',
                     guide = guide_legend(override.aes = list(alpha = 1))) +
...
```{r wide_annot_tick, fig.height = 6, fig.width = 6}
p_jit +
  scale_color_manual(values = cbep_colors2()[c(1,5,2)],
                     name = '',
                     guide = guide_legend(override.aes = list(alpha = 1))) +
...
We generate separate graphics, principally for use in PowerPoint.  These figures
are scaled to half a PowerPoint slide.  Since  RMARKDOWN saves a version of each
graphic in PNG format, we do not need to specifically save each figure.
...
We modify our code slightly to control the x axis on the Chlorophyll graphic to 
match the other figures.
